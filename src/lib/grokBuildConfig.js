...
const previousDefaultRegExp = /^# meai-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# meai-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via MeAI gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# meai-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# meai-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
