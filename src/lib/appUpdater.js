...
// Collect PIDs of all meai-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from meai install dir or wrapper cli.js
        const isAppProcess = lower.includes("meai") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("meai") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "meai");
  }
  return path.join(os.homedir(), ".meai");
}
...
// Resolve npx/meai binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx meai` — works regardless of global bin path changes after npm i -g
