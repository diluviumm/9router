...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-meai.env");
};
...
export async function GET() {
  ...
  const hasMeAI = hasMeAIConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_MEAI_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile meai",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["meai"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_MEAI_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "meai configuration removed from jcode",
    });
  } catch (error) {
