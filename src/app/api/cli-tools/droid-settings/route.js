...
// Check if settings has MeAI customModels
const hasMeAIConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:meai"));
};
...
// POST - Update MeAI customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing MeAI configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:meai"));
...
// DELETE - Remove MeAI customModels only (keep other settings)
