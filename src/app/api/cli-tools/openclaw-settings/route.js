...
// Check if settings has MeAI config
const hasMeAIConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["meai"];
};

// Read per-agent models.json and return current model id (without "meai/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["meai"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasMeAI: hasMeAIConfig(settings),
...
// POST - Update MeAI settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `meai/${model}`;

    // Remove all old meai/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("meai/"))
    ...
    // Add fresh meai models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`meai/${m}`] = {};
    });

    // Remove old meai model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("meai/")) {
    ...
    // Update models.providers.meai with all models
    settings.models.providers["meai"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `meai/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove MeAI settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove MeAI from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["meai"];
    ...
    // Remove meai models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("meai/"));
    ...
    // Reset agents.defaults.model.primary if it uses meai
    if (settings.agents?.defaults?.model?.primary?.startsWith("meai/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "MeAI settings removed successfully",
    });
  } catch (error) {
