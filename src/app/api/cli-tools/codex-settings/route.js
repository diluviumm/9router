...
// Check if config has MeAI settings
const hasMeAIConfig = (config) => {
  if (!config) return false;
  return config.includes("model_provider = \"meai\"") || config.includes("[model_providers.meai]");
};
...
// POST - Update MeAI settings (merge with existing config)
export async function POST(request) {
  try {
    ...
    // Update only MeAI related fields (api_key goes to auth.json, not config.toml)
    parsed.model = model;
    parsed.model_provider = "meai";

    // Update or create meai provider section (no api_key - Codex reads from auth.json)
    // Ensure /v1 suffix is added only once
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;
    // Custom providers ignore auth.json - the key must travel as a static header
    setNestedSection(parsed, "model_providers.meai", {
      name: "MeAI",
...
export async function DELETE() {
  try {
    ...
    // Remove MeAI related root fields only if they point to meai
    if (parsed.model_provider === "meai") {
      delete parsed.model;
      delete parsed.model_provider;
    }

    // Remove meai provider section
    deleteNestedSection(parsed, "model_providers.meai");
