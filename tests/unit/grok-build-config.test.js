...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("meai");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "meai-general-purpose",
      explore: "meai-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.meai\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.meai-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.meai-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# meai-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.meai-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("meai-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.meai-");
    expect(reset).not.toContain("meai-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("meai-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("meai-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("meai-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("meai-explore");
    expect(getGrokSubagentSlot("plan")).toBe("meai-plan");
