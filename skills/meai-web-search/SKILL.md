...
description: Web and X search via MeAI /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity / Xquik. Use when the user wants to search the web, find articles, or search public X posts.
---
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"MeAI open source","max_results":5}'
```
...
Add the Xquik API key in MeAI's provider settings. Xquik charges 1 credit per returned post. Continue a search by passing `pagination.next_cursor` as `provider_options.cursor`.
