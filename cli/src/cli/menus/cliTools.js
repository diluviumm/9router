...
async function buildCodexHeader() {
  ...
  const { installed, hasMeAI, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasMeAI, settings } = result.data;
  ...
  // Extract MeAI custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:meai-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract MeAI provider config
  const provider = settings?.models?.providers?.["meai"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("meai/") ? primary.replace("meai/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasMeAI, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use MeAI\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
