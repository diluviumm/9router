...
Despliega MeAI en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.meai` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/meai
sudo chown $USER:$USER /var/lib/meai
```
...
Crea `/etc/nginx/sites-available/meai`:
...
sudo ln -s /etc/nginx/sites-available/meai /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de MeAI
...
tar -czf meai-backup-$(date +%Y%m%d).tar.gz /var/lib/meai

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/meai-$(date +\%Y\%m\%d).tar.gz /var/lib/meai
```
...
```bash
# Verificar si MeAI está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/meai
chmod 755 /var/lib/meai
```
