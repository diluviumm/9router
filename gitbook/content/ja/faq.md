...
**MeAIは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**MeAIはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
**はい! MeAIは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.meai`)
...
**はい、MeAIはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.meai`(またはカスタム`DATA_DIR`)にローカル保存
- MeAIサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
