...
Tích hợp MeAI với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của MeAI.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- MeAI đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint MeAI:
...
3. Nhận giải thích AI qua MeAI

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng MeAI để hiểu context và thực hiện thay đổi
...
1. Xác minh MeAI đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh MeAI instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs MeAI để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng MeAI cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://9router.com`
2. Đảm bảo bạn đã cấu hình API key trong MeAI cloud dashboard
...
## Tích hợp với Tính năng MeAI

### Định tuyến Model

MeAI tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, MeAI tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua MeAI dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
