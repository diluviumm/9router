...
Integrate MeAI with Cline VSCode extension to route your AI requests through MeAI's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- MeAI running locally or cloud endpoint configured
...
Set the base URL to your MeAI endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your MeAI endpoint
...
3. The key should start with `sk-meai-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your MeAI configuration
...
4. Cline will use MeAI to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using MeAI
...
3. Get AI-powered explanations through MeAI

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use MeAI to understand context and make changes
...
1. Verify MeAI is running: `curl http://localhost:20128/health`
...
2. Verify your MeAI instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check MeAI logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use MeAI cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://9router.com`
2. Make sure you have configured your API key in the MeAI cloud dashboard
...
## Integration with MeAI Features

### Model Routing

MeAI automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, MeAI automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through MeAI dashboard:
- Total requests
- Token usage
- Cost per model
