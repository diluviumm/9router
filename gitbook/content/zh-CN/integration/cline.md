...
将 MeAI 与 Cline VSCode 扩展集成,通过 MeAI 的智能路由系统转发你的 AI 请求。

## 前置要求

- 已安装 Visual Studio Code
- 从 VSCode 市场安装了 Cline 扩展
- MeAI 本地运行或已配置云端 endpoint
...
**步骤:**
1. 在 **Base URL** 字段中输入你的 MeAI endpoint
2. 末尾必须包含 `/v1`
...
2. 在聊天输入框中输入消息
3. 按 Enter 发送
4. Cline 会通过 MeAI 处理你的请求

### 生成代码

1. 让 Cline 生成代码:"创建一个登录表单的 React 组件"
2. Cline 会通过 MeAI 生成代码
3. 检查并接受生成的代码
...
1. 在编辑器中选中代码
2. 让 Cline:"解释这段代码"
3. 通过 MeAI 获得 AI 驱动的解释

### 文件操作

1. 让 Cline 创建、修改或删除文件
2. Cline 会通过 MeAI 理解上下文并进行修改
3. 在接受前检查变更
...
1. 确认 MeAI 正在运行:`curl http://localhost:20128/health`
...
使用 MeAI 云端 endpoint 而非 localhost:

1. 在 Cline 设置中将 Base URL 设为:`https://9router.com`
2. 确保已在 MeAI 云端仪表盘中配置 API key
...
某个提供商失败时,MeAI 会自动回退到仪表盘中配置的备用提供商。

### 使用跟踪

通过 MeAI 仪表盘监控你的 Cline 使用:
- 请求总数
- Token 使用
- 每个模型的成本
- 提供商分布
