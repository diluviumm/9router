...
MeAI is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[MeAI Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.meai/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.meai/usage.json`, `~/.meai/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.meai/db.json`)
- usage stats: `~/.meai/usage.json`
- request log lines: `~/.meai/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[MeAI Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.meai` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/meai && npm run build`
- Build Docker image: `cd /root/dev/meai && docker build -t meai .`
