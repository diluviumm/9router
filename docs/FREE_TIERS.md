> **Catatan fork MeAI** — dokumen free-tier ini diadaptasi dari proyek **OmniRoute** (MIT) untuk katalog provider fork MeAI. Angka & katalog dibiarkan apa adanya sesuai sumber (audit 2 mingguan oleh upstream); sebagian besar link dashboard mungkin mengacu ke halaman asli OmniRoute. Sumber: `docs/reference/FREE_TIERS.md` di repo OmniRoute.
