<div align="center">
  <img src="../images/meai.png?1" alt="Bảng điều khiển MeAI" width="800"/>
...
Gói kho lưu trữ này là riêng tư (`meai-app`), vì vậy việc chạy từ nguồn/Docker là cách phát triển cục bộ mặc định.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - MeAI + Claude Code MIỄN PHÍ
  
[![Thiết lập MeAI + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập MeAI
...
MeAI hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → MeAI dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân MeAI **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của MeAI

**Thực tế Thanh toán MeAI:**

✅ **Phần mềm MeAI = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **MeAI không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, MeAI chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị MeAI tính phí không?</b></summary>

**Không.** MeAI là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, MeAI chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân MeAI** → **Không bao giờ tính phí bất cứ thứ gì, ever**

MeAI là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
MeAI chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và MeAI giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải MeAI.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - MeAI giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của MeAI ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và MeAI sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. MeAI theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển meai]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với MeAI cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.meai/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.meai` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
- Lịch sử sử dụng và log: `~/.meai/usage.json` và `~/.meai/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.meai` và độc lập với `DATA_DIR`.
...
- Runtime MeAI cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
Cảm ơn tất cả những người đã đóng góp giúp MeAI tốt hơn!
...
[![ Chart](https://starchart.cc/decolua/meai.svg?variant=adaptive)](https://starchart.cc/decolua/meai)
