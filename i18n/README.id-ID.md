...
Paket repo ini bersifat privat (`meai-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
### 📺 Panduan Setup Lengkap - MeAI + Claude Code Gratis
...
MeAI bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → MeAI mengonversi → provider menerima dalam format nativenya
...
> MeAI sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing MeAI!
...
### 📊 Tentang Biaya dan Penagihan MeAI

**Fakta soal penagihan MeAI:**

✅ **Software MeAI = gratis selamanya** (open source, tanpa tagihan)
...
❌ **MeAI tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, MeAI hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **MeAI**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat MeAI.
...
<details>
<summary><b>💳 Apakah MeAI menagih saya?</b></summary>

**Tidak.** MeAI adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, MeAI hanya me-routing request
- ❌ **MeAI sendiri** → **tidak menagih apa pun**

MeAI adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
MeAI hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan MeAI membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh MeAI.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - MeAI memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback MeAI mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan MeAI akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. MeAI melacak kuota per model!
...
```bash
npm install -g meai
PORT=20128 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com meai
```
...
**Kalau MeAI membantumu, kasih ⭐ di [GitHub](https://github.com/decolua/9router)!**

[🌐 Website](https://9router.com) • [📦 npm](https://www.npmjs.com/package/meai) • [🐛 Laporkan Bug](https://github.com/decolua/9router/issues)
