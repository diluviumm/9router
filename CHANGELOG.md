...
- **CLI tools**: send the API key Codex actually reads —
  `[model_providers.meai.http_headers]` instead of auth.json (which left
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point MeAI at your own
...
- **RTK**: `X-MeAI-Token-Saver` header to bypass token savers per request
...
## Docs
- README: add Indonesian MeAI tutorial video (#1709)
