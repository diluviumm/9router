<div align="center">
  ...
  # MeAI - FREE AI Router & Token Saver
...
This repository package is private (`meai-app`), so source/Docker execution is the expected local development path.
  ...
  <tr>
    ...
    <td align="center" width="320">
      <a href="https://www.youtube.com/watch?v=hPusYX-5Pmw">
        <img src="https://img.youtube.com/vi/hPusYX-5Pmw/maxresdefault.jpg" alt="Hướng Dẫn Setup OpenClaw + MeAI: Tạo Bot Zalo AI Tự Động Từ A-Z" width="300"/>
      </a><br/>
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Hướng Dẫn Setup OpenClaw + MeAI: Tạo Bot Zalo AI Tự Động Từ A-Z<br/>by <a href="https://github.com/tuanminhhole">tuanminhhole</a></sub>
    </td>
    <td align="center" width="320">
      <a href="https://www.youtube.com/watch?v=hgnE7MKi3Y4">
        <img src="https://img.youtube.com/vi/hgnE7MKi3Y4/maxresdefault.jpg" alt="Bye Limit! Cara Bikin Sistem 'AI Unlimited' 100% Gratis Dengan MeAI!
" width="300"/>
      </a><br/>
      <b>🇮🇩 Indonesia</b><br/>
      <sub>Bye Limit! Cara Bikin Sistem "AI Unlimited" 100% Gratis Dengan MeAI!<br/>by <a href="https://www.youtube.com/@neptiver">neptiver</a></sub>
    </td>
...
> 🎬 **Made a video about MeAI?** Submit a [Pull Request](https://github.com/decolua/9router/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

MeAI works seamlessly with all major AI coding tools:
...
Set `X-MeAI-Token-Saver: off` to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. MeAI calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:

```
Client → MeAI → Headroom /v1/compress → MeAI → provider
```
...
If Headroom is down or returns an error, MeAI fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → MeAI translates → Provider receives native format
...
> MeAI itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding MeAI Costs & Billing

**MeAI Billing Reality:**

✅ **MeAI software = FREE forever** (open source, never charges)  
...
❌ **MeAI never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, MeAI just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **MeAI**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through MeAI.
...
<details>
<summary><b>💳 Will I be charged by MeAI?</b></summary>

**No.** MeAI is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, MeAI just routes your requests
- ❌ **MeAI itself** → **Never charges anything, ever**

MeAI is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
MeAI just routes your requests to them - there's no "catch" or future billing from MeAI itself. They're truly free services, and MeAI makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - MeAI helps maximize their value through quota tracking
...
MeAI's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and MeAI respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. MeAI tracks quota per model!
...
> **Note:** OpenClaw only works with local MeAI. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
docker pull decolua/meai:latest   # update to latest
```

**Data persistence:** `$HOME/.meai/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.

### Environment Variables

| Variable                                             | Default                                  | Description                                                                         |
| ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Auto-generated (`~/.meai/jwt-secret`) | JWT signing secret for dashboard auth cookie (override to share across instances)   |
| `INITIAL_PASSWORD`                                   | `123456`                                 | First login password when no saved hash exists                                      |
| `DATA_DIR`                                           | `~/.meai`                             | Main app data location (SQLite at `$DATA_DIR/db/data.sqlite`)                       |
...
- Both `${DATA_DIR}` and `~/.meai` resolve to the same location in a Docker container — the symlink `/root/.meai -> /app/data` is created at build time.
...
Thanks to all contributors who helped make MeAI better!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of MeAI. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. MeAI ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. MeAI adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. MeAI injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, MeAI's token-saving features wouldn't exist. ⭐ them on GitHub!
